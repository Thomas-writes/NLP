'''
This file holds everything that is used for writing to the markdown files for each problem

This typically gets output from the solver when appending solutions and then outputs that to the md files
'''
...
def md_writer(filter, problem_name, convexity, obj_type):
    ...
    with open(f"./problems/{filter}/{bounds}/{problem_name}.md", "w") as f:
        ...
        f.write(f"- **Objective Type:** {obj_type}\n")
        ...
        for i in problem_names:
            ...
            with open(f"./problems/{filter}/{bounds}/{i}.md", "a") as f:
                while counter < len(successes):
                    start_vals[counter] = np.array2string(start_vals[counter], formatter={'float_kind': lambda x: f"{x:.5g}"}, max_line_width=10**9)
