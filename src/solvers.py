...
class solver:
    ...
    def _random_starts_t2(self, k=3):
        ...
        for _ in range(k):
            ...
            x = x0 + (.1 * scale * rng.standard_normal(size=x0.size))
            '''
            this is just a safety cap so values don't explode (since we don't have box bounds)
            '''
            x = np.minimum(np.maximum(x, -maximum), maximum)

            '''
            check if the random start is valid if it isnt try a different point maximum 50 times
            this will break if after 50 times it is still invalid
            '''
            for __ in range(50):
                try:
                    c = self.p.cons(x)
                    if np.all(c >= self.p.cl) and np.all(c <= self.p.cu):
                        break
                except Exception:
                    pass

                # try a new random perturbation
                x = x0 + (.1 * scale * rng.standard_normal(size=x0.size))
                x = np.minimum(np.maximum(x, -maximum), maximum)
